...
export const FOIRResult: React.FC<FOIRResultProps> = ({
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      watermark.style.cssText = `
        ...
        pointer-events: none;
      `;
      watermark.textContent = 'Generated by Loan Chacha';
      resultCardRef.current.style.position = 'relative';
      ...
      const canvas = await html2canvas(resultCardRef.current, {
        ...
        windowHeight: resultCardRef.current.offsetHeight
      });
      ...
      canvas.toBlob(async (blob) => {
        ...
        const file = new File([blob], 'foir-result.png', { type: 'image/png' });
        ...
        link.href = url;
        link.download = `FOIR_Result_${new Date().toISOString().split('T')[0]}.png`;
        ...
        toast({
          ...
          description: "Your FOIR result image has been saved"
        });
      }, 'image/png', 0.95);
      ...
      toast({
        title: "Share Failed",
        description: "Unable to generate or share image",
        variant: "destructive"
      });
    }
  };
  ...
  return (
    <Card 
      ref={resultCardRef}
      className={`card-elegant animate-bounce-in bg-gradient-to-br ${config.bgGradient} border-2 ${config.borderColor}`}
    >
